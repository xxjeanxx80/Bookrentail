...
$duration = '';
if (isset($_GET['id'])) {
    $bookId = pg_escape_string($con, $_GET['id']);
}
if (isset($_GET['duration'])) {
    $duration = pg_escape_string($con, $_GET['duration']);
}
...
if (isset($_POST['submit'])) {
    ...
    $sql = "INSERT INTO orders(user_id, address, address2, pin, payment_method, total, payment_status, order_status, date, duration)
            VALUES ('$userId', '$address', '$address2', '$pin','$paymentMethod','$totalPrice','$paymentStatus','$orderStatus','$date','$duration') RETURNING id";
    $result = pg_query($con, $sql);
    $row = pg_fetch_assoc($result);
    $orderId = $row['id'];
    
    $productId = $getProduct['0']['id'];
    pg_query($con, "INSERT INTO order_detail(order_id,book_id,price,time)
            ...
            <div class="col-md-5 col-lg-4 order-md-last">
                ...
                <ul class="list-group mb-3 dark-background">
                    ...
                    <li class="list-group-item justify-content-start lh-sm">
                        <p><span class="fw-bold">Rent Price</span> = ₫<?php echo number_format($getProduct['0']['rent']) ?> Per Day
                        </p>
                        <p><span class="fw-bold">Duration</span> = <?php echo $duration ?> Days</p>
                        <p><span class="fw-bold">Total Rent</span> = ₫<?php echo number_format($totalRent) ?></p>
                        <p><span class="fw-bold">Security Deposit<span style="color: red"><sup>*</sup></span></span> =
                            ₫<?php echo number_format($getProduct['0']['security']) ?></p>
                        <p><span class="fw-bold">Total price</span> = ₫<?php echo number_format($totalPrice) ?> </p>
                    </li>
                </ul>
